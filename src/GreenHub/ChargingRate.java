...
public class ChargingRate implements Serializable {
	...
	private int id;
	private int power; // Potenza in kW
	private double price; // Prezzo per kWh
	...
	// Metodo che restituisce il prezzo per kWh
	public double getRatePerKWh() {
		return price; // Restituisce il prezzo per kWh
	}
	...
	public static void printAll1(ArrayList < ChargingRate > chargingRateList) {
		...
		int i = 1;
		for (ChargingRate cr: chargingRateList) {
			System.out.println(i + ") ID: " + cr.getId() + ", Power: " + cr.getPower() + " kW, Price: " + cr.getRatePerKWh() + " €/kW");
			i++;
		}
	}
}